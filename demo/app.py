...
DEFAULT_PARAMS_3 = EncodeParams(k=7, rs_overhead=2.0, min_margin=0.05)
DEFAULT_PARAMS_2 = EncodeParamsB(k=7, rs_overhead=2.0, min_margin=0.05, iou_threshold=0.5)


def _detect_method(sidecar_path: str) -> str:
    """Read the first 4 bytes of a sidecar file to determine its method."""
...
def run_encode(image_path, message, key, method):
    ...
    try:
        if method == "Option 2 — IoU matching (larger sidecar, more accurate)":
            sidecar = encode_watermark_b(image, msg_bytes, key_int, DEFAULT_PARAMS_2)
            ...
            sidecar = encode_watermark(image, msg_bytes, key_int, DEFAULT_PARAMS_3)
...
def run_decode(altered_image_path, sidecar_file, original_image_path):
    ...
    # Extract actual file path from Gradio file object
    ...
    # Auto-detect sidecar method from magic bytes
    detected = _detect_method(sidecar_path)  # ← Now passing a string path
...
def _decode_option2(after, sidecar_path, original_image_path):
    ...
    # Resize after-image the same way the decoder does.
    ...
    # Diagnostics with original image.
    raw_output = None
    ...
    if original_image_path is not None:
        ...
        if orig is not None:
            ...
            # Compare against expected bits from original descriptor ordering.
    ...
    with gr.Tab("Encode"):
        ...
        with gr.Row():
            with gr.Column():
                ...
                enc_message = gr.Textbox(
                    label="Message to encode",
                    placeholder="hi im a dog named bruno",
                )
        ...
        enc_btn.click(
            fn=run_encode,
            inputs=[enc_image, enc_message, enc_key, enc_method],
